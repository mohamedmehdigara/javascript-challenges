...
test('it calculates an integer square root', assert => {
  ...
  const expected = 4;
  assert.equal(actual.toFixed(6), expected.toFixed(6), 'should get 4 as the square root of 16');
  assert.end();
});
...
test('it calculates a decimal square root', assert => {
  ...
  assert.equal(actual.toFixed(6), expected.toFixed(6), 'should ...');
  assert.end();
});
...
test('it produces synonymous results with JS native Math.sqrt fn', assert => {
  ...
  assert.equal(actual.toFixed(6), expected.toFixed(6), 'should be the same result as Math.sqrt');
  assert.end();
});
...
test('it calculates infinite fraction square roots', assert => {
  ...
  assert.equal(actual.toFixed(6), expected.toFixed(6), 'should properly end infinite decimal square roots');
  assert.end();
});
