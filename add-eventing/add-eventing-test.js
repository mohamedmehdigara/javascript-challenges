...
// add-eventing-test.js
const addEventing = require('./add-eventing'); // Adjust the path if needed
...
test('if it is called with multiple arguments it should pass them to the listeners', assert => {
  const obj = addEventing({ name: 'Kelsey', age: 30 });
  const ageChangeEvent = sinon.spy();
  const newName = 'Alice';
  const newAge = 31;
